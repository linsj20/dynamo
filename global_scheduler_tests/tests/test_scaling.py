...
class ScalingTest(BaseGlobalSchedulerTest):
    ...
    async def send_request_quiet(self, request: TestRequest) -> TestRequest:
        """Send a single test request with streaming enabled for TTFT/TPOT metrics using HTTP v1/chat/completions"""
        # Use the base class HTTP implementation but without verbose logging
